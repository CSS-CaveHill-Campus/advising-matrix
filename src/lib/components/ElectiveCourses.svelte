<script lang="ts">
	...
	onMount(() => {
		const storedCourses = localStorage.getItem('electiveCourses');
		if (storedCourses) {
			cours = JSON.parse(storedCourses);
		}
	});

	$: electiveCourses = data.electiveCourses.filter(
	...
	const addElectiveCourses = (event: MouseEvent) => {
		...
		const course = data.electiveCourses.find((course) => course.id === selectedCourse.value);
		if (!course) return;
		cours = [...cours, course];
...
<!-- <pre>{JSON.stringify(data.electiveCourses,null,2)}</pre> -->
